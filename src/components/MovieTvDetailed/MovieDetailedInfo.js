				...
				<Grid container spacing={4}>
					...
					<Grid item md={7}>
						<div>
							...
							{renderTrailer(movie.videos.results, modalOpened, setModalOpened, setmodalClosed)}
...
function renderTrailer(videos, modalOpened, setmodalOpened, setmodalClosed) {
	...
	return (
		<React.Fragment>
			<Button variant="contained" color="secondary" style={{ marginLeft: '30px' }} onClick={setmodalOpened}>
				Trailer
				<PlayArrowIcon />
			</Button>

			<ModalVideo channel="youtube" isOpen={modalOpened} videoId={key} onClose={setmodalClosed} />
		</React.Fragment>
	);
}
